  ...
  << " [A]poyar Petición\n [S]Cerrar Sesión\n [X]Salir"<<std::endl;
      ...
      switch (option) {
        ...
        case 'M':
          ...
          // Mostrar las listas
          std::cout <<  Data_base.GetUser(user_name).getCreatedPetitions() << std::endl;
          break;
        case 'A':
          std::cout << "¿Qué petición quiere apoyar?" << std::endl;
          std::cin >> search_word;
          // Buscar peticion y firmar
          //comparariamos si el titulo existe nos faltaria la clase para hacerlo
